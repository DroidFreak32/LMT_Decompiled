...
class Action {
    ...
    void checkNeededPermissions(Activity activity, boolean request) {
        if (getType() == Action.Shortcut  && this.mString.contains("android.intent.action.CALL"))
            PermissionChecker.getInstance().checkAndRequestPhoneCallPermission(activity, true);
        else if (getType() == Action.PieRecentApps && Build.VERSION.SDK_INT >= 21 && !RootContext.getInstance(activity).isRootAvailable(false))
